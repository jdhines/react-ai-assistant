...
export interface Props {
	...
	onActionOpenUrl?: (action: AdaptiveCards.OpenUrlAction) => void;
	onActionShowCard?: (action: AdaptiveCards.ShowCardAction) => void;
...
export const AdaptiveCard = ({
		...
		(a: AdaptiveCards.Action) => {
			...
			switch (type) {
				...
				case AdaptiveCards.ShowCardAction.JsonTypeName: {
					if (onActionShowCard) {
						onActionShowCard(a as AdaptiveCards.ShowCardAction);
					}
					break;
				}
				case AdaptiveCards.SubmitAction.JsonTypeName: {
					if (onActionSubmit) {
						onActionSubmit(a as AdaptiveCards.SubmitAction);
					}
					break;
				}
			}
	...
	useEffect(() => {
		if (!targetRef.current) {
			console.error(
				"Target reference is not set. Ensure the component is mounted correctly.",
			);
			return;
		}
		...
		try {
			...
			if (onError) {
				onError(cardRenderError as Error);
				setError(cardRenderError as Error);
			}
		}
	}, [payload, onError]);
